return {
	{ -- LSP Configuration & Plugins
		...
		config = function()
			...
			local custom_lua_ls = vim.fn.expand("~/.local/lsp_servers/lua-language-server/bin/lua-language-server")

			-- Use custom one if it exists, otherwise fallback to system/default
			local lua_ls_cmd = vim.fn.filereadable(custom_lua_ls) == 1 and { custom_lua_ls } or nil -- fallback to default if nil
			local lua_config = {
				settings = {
					Lua = {
			...
			if vim.fn.filereadable(custom_lua_ls) == 1 then
				lua_config.cmd = { custom_lua_ls }
			end
